...
void column_step(life_t *l, int num);
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < l.steps; i++) {
		if ((i % l.save_steps == 0) && rank == numprocess-1) {
			sprintf(buf, "res_cols/life_%06d.vtk", i);
...
void life_init(const char *path, life_t *l)
{
    ...
    MPI_Type_vector(l->ny, 1, l->nx, MPI_INT, &(l->column));
...
void life_step(life_t *l, int rank, int numprocess)
{
    ...
    int count = l->nx / numprocess;
    int count_last = count + (l->nx % numprocess); 
    ...
    while(num < N)
    {
        column_step(l, num + rank * count);
        num = num + 1;
    }

    if (rank != numprocess-1)
    {
        int* sdata = l->u1 + rank * count;
        for (j = 0; j < count; j++)
        {
            MPI_Send(sdata+j, 1, l->column, numprocess - 1, 0, MPI_COMM_WORLD);
        }
    }
    else
    {
        ...
        {   
            for (j = 0; j < count; j++)
            {
                MPI_Recv(l->u1 + j + i*count, 1, l->column, i, 0, 
        ...
        for (i = 0; i < numprocess -1; i++)
        {   
            MPI_Send(l->u1, l->nx*l->ny, MPI_INT, i, 0, MPI_COMM_WORLD);
        }
    }
    if (rank != numprocess-1)
    {
        MPI_Recv(l->u1, l->nx*l->ny, MPI_INT, numprocess - 1, 0, 
                               MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
	int *tmp;
	tmp = l->u0;
